...
export default function CoachPerformancePage() {
  ...
  const loadAttributes = useCallback(async (sportId) => {
    ...
    try {
      const result = await coachGet(`/coach/performance/attributes?sport_id=${sportId}&status=APPROVED`);
      const responseData = result.data;
      if (Array.isArray(responseData)) {
  ...
  return (
    <div className="space-y-6 p-6 bg-surface min-h-screen">
      <div>
        ...
        <p className="text-sm text-muted">Evaluate student performance across sports attributes.</p>
      </div>
      ...
      {/* Batch Selection */}
      <div className="card">
        <h3 className="mb-4 font-bold text-foreground">Select Batch</h3>
        {batches.length > 0 ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {batches.map((batch) => (
              <button
                ...
                className={`p-4 text-left transition-all duration-300 hover:border-accent/30 hover:-translate-y-0.5 rounded-lg border ${
                  selectedBatch?.batch_id === batch.batch_id ? 'border-accent bg-accent/5' : 'border-border bg-surface-secondary'
                }`}
              >
                <div className="font-bold text-foreground">{batch.name}</div>
      ...
      {selectedBatch && (
        <>
          {/* Student Selection */}
          <div className="card">
            <h3 className="mb-4 font-bold text-foreground">Select Student</h3>
            {students.length > 0 ? (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {students.map((student) => (
                  <button
                    ...
                    className={`p-4 text-left transition-all duration-300 hover:border-accent/30 hover:-translate-y-0.5 rounded-lg border ${
                      selectedStudent?.student_id === student.student_id ? 'border-accent bg-accent/5' : 'border-border bg-surface-secondary'
                    }`}
                  >
            ...
            <>
              ...
              <div className="card">
                ...
                <div className="space-y-4">
                  {attributes.map((attr) => (
                    <div key={attr.attribute_id} className="p-4 bg-surface-secondary rounded-lg border border-border">
                      <div className="mb-3">
                ...
                <div className="flex gap-4 mt-6 pt-4 border-t border-border">
                  <button
                    ...
                    className="bg-accent text-white hover:bg-accent-hover px-6 py-3 rounded-xl font-bold text-sm transition-all duration-300"
                  >
                    Transmit Report to Admin Portal
                  </button>
                  <button
                    onClick={handleDownloadReport}
                    className="btn-secondary px-6 py-3 rounded-xl font-bold text-sm transition-all duration-300"
                  >
              ...
              <div className="card">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-foreground">Propose New Tracking Attribute</h3>
                  <button
                ...
                {showNewProposal && (
                  <form className="space-y-4" onSubmit={handleProposeAttribute}>
                    <div>
                      <label className="label" htmlFor="newAttrSport">Sport</label>
                      <select
                        id="newAttrSport"
                    ...
                    <div>
                      <label className="label" htmlFor="newAttrName">Attribute Name</label>
                      <input
                        id="newAttrName"
                        type="text"
                        className="input-field"
                    ...
                    <button
                      type="submit"
                      className="bg-accent text-white hover:bg-accent-hover px-4 py-2 rounded-lg font-bold text-sm transition-all duration-300"
                    >
                      Submit for Approval
                    </button>
                  </form>
          ...
          {selectedStudent && attributes.length === 0 && (
            <div className="card">
              <p className="text-muted text-sm">
                No performance attributes defined for this sport yet. Contact your academy admin to add attributes.
              </p>
            </div>
          )}
        </>
