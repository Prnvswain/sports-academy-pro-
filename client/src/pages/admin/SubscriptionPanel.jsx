...
import { 
  ...
  QrCode, Clipboard, Gift, Clock, Sparkles, ShieldCheck, RefreshCw, X
} from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] py-20 gap-3">
        ...
        <p className="text-muted-foreground text-sm">Synchronizing plan usage metrics...</p>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-emerald-600 via-teal-600 to-cyan-700 border border-emerald-500/30 p-6 rounded-2xl relative overflow-hidden shadow-lg">
          ...
          <div className="relative z-10 mt-8 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-white/20 pt-6">
            {/* Coach Limit */}
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-white/80 font-semibold">Coaches Enrolled</span>
                <span className="text-white font-bold">{details?.teacher_usage} / {details?.teacher_limit || 'Unlimited'}</span>
              </div>
              <div className="w-full bg-black/20 rounded-full h-2">
                <div 
                  className={`h-2 rounded-full transition-all duration-500 ${coachProgress >= 90 ? 'bg-red-400' : 'bg-lime-300'}`} 
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-white/80 font-semibold">Active Students</span>
                <span className="text-white font-bold">{details?.student_usage} / {details?.student_limit || 'Unlimited'}</span>
              </div>
              <div className="w-full bg-black/20 rounded-full h-2">
                <div 
                  className={`h-2 rounded-full transition-all duration-500 ${studentProgress >= 90 ? 'bg-red-400' : 'bg-lime-300'}`} 
              ...
              <ul className="space-y-1.5 text-xs text-muted-foreground">
                {details.plan_features.slice(0, 3).map((f, i) => (
                  <li key={i} className="flex items-center gap-2">
                    <Check className="h-3.5 w-3.5 text-lime-500 shrink-0" /> {f}
                  </li>
                ))}
              </ul>
            )}
      ...
      <div id="plans-grid" className="space-y-4">
        <div className="text-center max-w-2xl mx-auto space-y-2">
          <h2 className="text-2xl font-extrabold text-foreground tracking-tight">Available Subscription Plans</h2>
          <p className="text-muted-foreground text-sm">Select the best plan that matches your sports program. Upgrade anytime dynamically.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((p) => {
            ...
            return (
              <div 
                key={p.id} 
                className={`bg-card border rounded-2xl p-6 flex flex-col justify-between relative transition-all shadow-sm ${
                  isCurrent 
                    ? 'border-lime-500 shadow-lg shadow-lime-500/10 ring-1 ring-lime-500/20' 
                    : 'border-border hover:border-primary/40 hover:shadow-md'
                }`}
              >
                {isCurrent && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-lime-500 text-white font-bold px-3 py-0.5 rounded-full text-xs uppercase tracking-wider shadow">
                    Current Plan
                  </span>
                )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {p.highlights?.[0] && (
                      <p className="text-lime-600 dark:text-lime-400 text-xs font-semibold mt-1">{p.highlights[0]}</p>
                    )}
                  </div>

                  <div className="py-2">
                    <p className="text-3xl font-extrabold text-foreground">₹{p.price}</p>
                    <p className="text-xs text-muted-foreground mt-1 font-semibold uppercase tracking-wider">Billed {p.duration}</p>
                  </div>

                  <div className="space-y-2 pt-2 border-t border-border text-sm">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Coaches Limit:</span>
                      <span className="text-foreground font-bold">{p.teacher_limit || 'Unlimited'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Students Limit:</span>
                      <span className="text-foreground font-bold">{p.student_limit || 'Unlimited'}</span>
                    </div>
                  </div>
                ...
                <button
                  ...
                  className={`w-full mt-6 py-2.5 rounded-xl font-bold transition-all text-center ${
                    isCurrent 
                      ? 'bg-surface-secondary text-muted-foreground border border-border cursor-default hover:text-foreground' 
                      : p.price === 0 
                        ? 'bg-surface-secondary hover:bg-border text-foreground border border-border'
                        : 'bg-lime-500 hover:bg-lime-400 text-white hover:shadow-[0_4px_15px_rgba(132,204,22,0.3)]'
                  }`}
                >
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
          <Clock className="text-lime-500 h-5 w-5" /> Subscription Payment Logs
        </h3>
        <div className="bg-card rounded-2xl border border-border overflow-x-auto shadow-sm">
          <table className="w-full text-left min-w-[700px]">
            <thead>
              <tr className="border-b border-border text-muted-foreground text-xs font-bold uppercase tracking-wider bg-surface-secondary/50">
            ...
            <tbody>
              {!details?.payment_history || details.payment_history.length === 0 ? (
                ...
                details.payment_history.map((tx) => (
                  <tr key={tx.id} className="border-b border-border/50 text-sm text-foreground hover:bg-surface-secondary/30 transition-colors">
                    <td className="p-4">
                      <p className="font-semibold text-foreground">{tx.plan_name}</p>
                      <p className="text-xs text-muted-foreground">Method: {tx.payment_method} {tx.coupon_code && `• Coupon: ${tx.coupon_code}`}</p>
                    </td>
                    <td className="p-4 font-mono text-xs text-muted-foreground">{tx.transaction_id || 'N/A'}</td>
                    <td className="p-4 font-bold text-foreground">₹{tx.amount}</td>
                    <td className="p-4 text-xs text-muted-foreground">{new Date(tx.created_at).toLocaleString()}</td>
                    <td className="p-4">
        ...
        {checkoutPlan && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="bg-card border border-border p-6 rounded-2xl w-full max-w-lg relative z-10 space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto [&::-webkit-scrollbar]:hidden"
            >
              {/* Header */}
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-xl font-extrabold text-foreground flex items-center gap-2">
                    <Sparkles className="text-lime-500 h-5 w-5 animate-pulse" /> Complete Subscription
                  </h3>
                  <p className="text-muted-foreground text-xs mt-0.5">Transfer funds and enter checkout verification ID.</p>
                </div>
                <button 
                  onClick={() => setCheckoutPlan(null)}
                  className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded-lg hover:bg-surface-secondary"
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              {/* Order Summary */}
              <div className="bg-surface-secondary border border-border p-4 rounded-xl space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Order:</span>
                  <span className="text-foreground font-bold">{checkoutPlan.name}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Amount:</span>
                  <span className="text-foreground font-bold">₹{checkoutPlan.price}</span>
                </div>
                {couponApplied && (
                ...
                <div className="flex justify-between text-base font-bold border-t border-border pt-2">
                  ...
                  <span className="text-lime-600 dark:text-lime-400">₹{Math.max(0, checkoutPlan.price - (checkoutPlan.price * couponDiscount) / 100)}</span>
                </div>
              </div>

              {/* QR Gate & Details */}
              {paymentSettings?.qr_enabled && (
                <div className="flex flex-col items-center gap-2 p-3 bg-surface-secondary rounded-xl border border-border">
                  <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Scan to pay instantly</p>
                  ...
                  <p className="text-[10px] text-muted-foreground">Supported by all standard UPI Apps</p>
                </div>
              )}

              {/* UPI & Transfer Info */}
              <div className="space-y-3">
                <div className="flex justify-between items-center p-3 bg-surface-secondary rounded-xl border border-border text-sm">
                  <div className="min-w-0 flex-1">
                    <p className="text-muted-foreground text-xs">UPI Address:</p>
                    <p className="font-bold text-foreground truncate">{paymentSettings?.upi_id || 'merchant@upi'}</p>
                    {paymentSettings?.merchant_name && (
                      <p className="text-[10px] text-muted-foreground font-semibold">{paymentSettings.merchant_name}</p>
                    )}
                  </div>
                  <button 
                    onClick={handleCopyUpi}
                    className="p-2 bg-surface hover:bg-border text-muted-foreground hover:text-foreground rounded-lg transition-all border border-border ml-3"
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full bg-surface border border-border rounded-xl py-2 pl-9 pr-3 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-lime-500/50 transition-colors"
                    />
                  </div>
                  <button
                    type="button"
                    onClick={handleApplyCoupon}
                    className="px-3 bg-surface-secondary hover:bg-border text-foreground rounded-xl text-xs font-bold transition-all shrink-0 border border-border"
                  >
                    Apply
                  </button>
                </div>
              ...
              {/* Checkout Form */}
              <form onSubmit={handleCheckoutSubmit} className="space-y-4 border-t border-border pt-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-surface border border-border rounded-xl py-2.5 px-4 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-lime-500 transition-colors"
                  />
                </div>
                ...
                <div className="flex items-center gap-2.5 py-1">
                  <input
                    ...
                    className="h-3.5 w-3.5 text-lime-500 rounded border-border focus:ring-0 cursor-pointer accent-lime-500"
                  />
                  <label htmlFor="auto_approve" className="text-xs font-bold text-lime-600 dark:text-lime-400 select-none cursor-pointer flex items-center gap-1">
